...
/*
Every class in Java inherits from the class named Object either directly or indirectly.
(The only exception is the Object class itself -- it does not inherit from anything.)
If a class does not explicitly extend another class, then it implicitly extends the Object class.
For example, the Person class, below, directly inherits from the Object class.
If Student had a subclass C, then C would directly extend Student and indirectly extend Object.
...
class PersonWithOverloadedEquals {
    ...
    This method does not override the equals method of the Object class, since the equals method of the Object class has a parameter of type Object, not of type Person.
    In fact, we are overLOADING the equals method -- we're writing another method with the same name but different parameter type.
...
class PersonWithOverriddenEquals {
    ...
    public boolean equals(Object o) {
        ...
        // return this.name.equals(other.name) && this.age == other.age;
        // with the cast, it compiles, and it runs well in many cases, but will cause a ClassCastException if o refers to some other kind of object, which it might
...
/*
When we extend a class, we must make sure that our class does not violate requirements of the superclass.
...
The specification of the hashCode() method states: if o1.equals(o2), then we must make sure that o1.hashCode() == o2.hashCode().
If we override equals, making distinct objects considered equal, we are in violation hashCode()'s requirement, unless we also override hashCode().
Bottom line: if you override equals, you almost certainly have to override hashCode() as well.
...
But this approach leads to slow performance of HashSet and HashMap (covered later in this course).
The best approach is to create an int based on the fields. Doing this yourself is not easy, but there's a shortcut: Objects.hash(...)

We can pass as many fields as we want to Objects.hash(...), and it will return an integer based on the fields.
The fields that we send to this method should be the same as those that are used in the equals method.
When we use Objects.hash(...) correctly, we are guaranteed to have equal hash codes for equal objects.
Additionally, it is very, very likely that unequal objects will have unequal hash codes, but this cannot be guaranteed.
